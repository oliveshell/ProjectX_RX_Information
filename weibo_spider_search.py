...
class Spider(object):
    ...
    def LoginWeibo(self,username, password):
        try:
            ...
            elem_sub = self.driver.find_element_by_xpath("//input[@class='smb_btn']")
    ...
    def handlePage(self):
        while True:
            ...
            if self.checkContent():
                ...
                if self.checkNext():
                    #拿到下一页按钮
                    next_page_btn = self.driver.find_element_by_xpath("//a[@class='page next S_txt1 S_line1']")
                    next_page_btn.click()
                else:
                    print "no Next"
    ...
    #但没有内容的前提是有“pl_noresult”
        try:
            self.driver.find_element_by_xpath("//div[@class='pl_noresult']")
            flag = False
        except:
            flag = True
        return flag
    ...
    def checkNext(self):
        try:
            self.driver.find_element_by_xpath("//a[@class='page next S_txt1 S_line1']")
            flag = True
        except:
            flag = False
        return flag
    ...
    #在添加每一个sheet之后，初始化字段
    def initXLS(self):
        name = ['博主昵称', '博主主页', '微博认证', '微博达人', '微博内容','微博图片', '发布时间', '微博地址', '微博来源', '转发', '评论', '赞']
    ...
    def getContent(self):
        ...
        #寻找到每一条微博的class
        nodes = self.driver.find_elements_by_xpath("//div[@class='WB_cardwrap S_bg2 clearfix']")
